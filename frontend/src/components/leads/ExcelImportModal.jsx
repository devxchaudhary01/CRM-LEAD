...
export default function ExcelImportModal({ onClose, onImported }) {
    ...
    const autoMap = headers => {
        ...
        headers.forEach(col => {
            ...
            if (!auto.emailId && lo.includes('email'))
                auto.emailId = col
            ...
            if (!auto.product && lo.includes('product'))
                auto.product = col

            if (!auto.service && lo.includes('service'))
                auto.service = col
        })
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 8, maxHeight: 260, overflowY: 'auto', paddingRight: 4 }}>
                            {columns.map((col, i) => (
                                <div key={i} onClick={() => toggle(col)} style={{
                                    ...
                                    <div style={{
                                        width: 18, height: 18, borderRadius: 5, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s',
