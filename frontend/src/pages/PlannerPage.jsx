...
import { RiArrowLeftLine, RiArrowRightLine, RiPhoneLine } from "react-icons/ri";
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600 }}>
                        {OUTCOMES[lead.c1.outcome]}
                      </span>
                      {lead.c1.date && (
                        <span style={{ fontSize: 10, color: "var(--muted)" }}>
                          {new Date(lead.c1.date).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                    {lead.c1.notes && (
...
export default function PlannerPage() {
  ...
  const getLeadsForDate = (date) => {
    ...
    return allLeads.filter(
      (lead) => getDateKey(new Date(lead.followUpDate)) === dateKey,
    );
  };
  ...
  const calendarDays = [];
  const totalSlots = Math.ceil((firstDay + monthDays) / 7) * 7;
    ...
    <div style={{ padding: "20px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--muted)" }}>
          View and manage scheduled follow-ups across your calendar
        </p>
      </div>
      ...
      >
        ...
        >
          <div style={{ fontSize: 24, fontWeight: 700, color: "#3B6FFF" }}>
            {allLeads.length}
          </div>
        ...
        >
          <div style={{ fontSize: 24, fontWeight: 700, color: "#00C48C" }}>
            {
              allLeads.filter((l) => new Date(l.followUpDate) <= new Date())
                .length
            }
          </div>
                      ...
                      >
                        ...
                        {dayLeads.length > 2 && (
                          <div style={{ fontSize: 10, color: "var(--muted)" }}>
                            +{dayLeads.length - 2} more follow-up
                            {dayLeads.length - 2 !== 1 ? "s" : ""}
                          </div>
                        )}
                      </div>
                    ) : (
